...
class HandGestureLoginApp:
  ...
  # Add scrollbar to user treeview
        user_tree_frame = tk.Frame(user_list_frame, bg="#2c3e50")
        user_tree_frame.pack(fill=tk.BOTH, expand=True)
        
        user_tree_scrollbar = ttk.Scrollbar(user_tree_frame)
        user_tree_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.user_tree = ttk.Treeview(user_tree_frame, columns=("Name", "Status"), show="headings", height=15,
    ...
    # ... (Include all the other methods from the previous working version)
    # For brevity, I've included the essential structure. The complete implementation
    # would include all the methods like start_live_camera_feed, update_frame, etc.
